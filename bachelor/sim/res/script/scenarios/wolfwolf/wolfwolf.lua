...
scenario.name = "Wolf Wolf"

-- Describe the scenario with a sensible amount of text
scenario.description = "There are wolfs running around to eat deers?"
...
scenario.init = function()
    ...
    -- Spawn wolfs
    for i = 1,10 do
        local wolf = cultsim.spawn("wolf")

        local need_comp = cultsim.get_component(wolf, component.need)
        for i, need in ipairs(need_comp.needs) do
            need.status = random:normal(75.0,100.0)
        end
    end
