...
// Businesses: you can support continued development via invoiced technical support, maintenance and sponsoring contracts. Please
// reach out to "contact AT dearimgui.org". Individuals: you can support continued development via donations. See docs/README or
// web page.
...
/*
 ...
 - Dear ImGui is a "single pass" rasterizing implementation of the IMGUI paradigm, aimed at ease of use and high-performances.
   For every application frame your UI code will be called only once. This is in contrast to e.g. Unity's own implementation of an
 ...
 - You can later customize the imconfig.h file to tweak some compile-time behavior, such as integrating Dear ImGui types with your
 own maths types.
 ...
 vs "render" phases of your own application. All rendering information are stored into command-lists that you will retrieve after
 ...
 bindings) io.DeltaTime = 1.0f/60.0f;              // set the time elapsed since the previous frame (in seconds) io.DisplaySize.x
 = 1920.0f;             // set the current display width io.DisplaySize.y = 1280.0f;             // set the current display height
 here io.MousePos = my_mouse_pos;             // set the mouse position io.MouseDown[0] = my_mouse_buttons[0];  // set the mouse
 button states io.MouseDown[1] = my_mouse_buttons[1];
 ...
 draw_data->DisplayPos + draw_data->DisplaySize)
                 //   In a single viewport application, draw_data->DisplayPos will always be (0,0) and draw_data->DisplaySize will
 always be == io.DisplaySize.
 ...
 (size being Max-Min) ImVec2 pos = draw_data->DisplayPos; MyEngineScissor((int)(pcmd->ClipRect.x - pos.x), (int)(pcmd->ClipRect.y
 - pos.y), (int)(pcmd->ClipRect.z - pos.x), (int)(pcmd->ClipRect.w - pos.y));
 ...
 values: 0.0f= not held. 1.0f= fully held. Pass intermediate 0.0f..1.0f values for analog triggers/sticks.
    - We uses a simple >0.0f test for activation testing, and won't attempt to test for a dead-zone.
      Your code will probably need to transform your raw inputs (such as e.g. remapping your 0.2..0.9 raw input range to 0.0..1.0
 imgui range, etc.).
 ...
 back-end NEEDS to move the OS or underlying mouse cursor on the next frame. Some of the binding in examples/ do that. (If you set
 the NavEnableSetMousePos flag but don't honor 'io.WantSetMousePos' properly, imgui will misbehave as it will see your mouse as
 moving back and forth!) (In a setup when you may not have easy control over the mouse cursor, e.g. uSynergy.c doesn't expose
 moving remote mouse cursor, you may want to set a boolean to ignore your other external mouse positions until the external source
 is moved again.)
 ...
 comments or references in all imgui files. You can read releases logs https://github.com/ocornut/imgui/releases for more details.
 ...
 - 2019/11/21 (1.74) - ImFontAtlas::AddCustomRectRegular() now requires an ID larger than 0x110000 (instead of 0x10000) to conform
 with supporting Unicode planes 1-16 in a future update. ID below 0x110000 will now assert.
 ...
 triggering on: 0.0 and (delay+rate*N) where (N>=1). Fixed formula responds to (N>=0). Effectively it made io.KeyRepeatRate behave
 like it was set to (io.KeyRepeatRate + io.KeyRepeatDelay). If you never altered io.KeyRepeatRate nor used GetKeyPressedAmount()
 ...
 ImGuiSetCond_*, IsItemHoveredRect(), IsPosHoveringAnyWindow(), IsMouseHoveringAnyWindow(), IsMouseHoveringWindow(),
 IMGUI_ONCE_UPON_A_FRAME. Grep this log for details and new names, or see how they were implemented until 1.71.
 - 2019/06/07 (1.71) - rendering of child window outer decorations (bg color, border, scrollbars) is now performed as part of the
 parent window. If you have overlapping child windows in a same parent, and relied on their relative z-order to be mapped to their
 submission order, this will affect your rendering. This optimization is disabled if the parent window has no visual output,
 because it appears to be the most common situation leading to the creation of overlapping child windows. Please reach out if you
 are affected.
 ...
 imstb_rectpack.h. If you were conveniently using the imgui copy of those STB headers in your project you will have to update your
 include paths.
 ...
 - 2018/06/06 (1.62) - renamed GetGlyphRangesChinese() to GetGlyphRangesChineseFull() to distinguish other variants and discourage
 using the full set.
 ...
 - 2018/05/03 (1.61) - DragInt(): the default compile-time format string has been changed from "%.0f" to "%d", as we are not using
 integers internally any more. If you used DragInt() with custom format strings, make sure you change them to use %d or an
 ...
 - 2018/03/12 (1.60) - removed ImGuiCol_CloseButton, ImGuiCol_CloseButtonActive, ImGuiCol_CloseButtonHovered as the closing cross
 uses regular button colors now.
 ...
 window (e.g. click-dragging item from another window to this window). You can use the newly introduced IsWindowHovered() flags to
 requests this specific behavior if you need it.
 ...
 ImVec2(-1,-1) was enough but we now accept negative mouse coordinates. In your binding if you need to support unavailable mouse,
 ...
 function (will obsolete).
                     - renamed IsMouseHoveringWindow() to IsWindowRectHovered() for consistency. Kept inline redirection function
 (will obsolete).
 - 2017/08/20 (1.51) - renamed GetStyleColName() to GetStyleColorName() for consistency.
 - 2017/08/20 (1.51) - added PushStyleColor(ImGuiCol idx, ImU32 col) overload, which _might_ cause an "ambiguous call" compilation
 error if you are using ImColor() with implicit cast. Cast to ImU32 or ImVec4 explicily to fix.
 ...
 affect you, otherwise if <1.0f you need tweak your custom theme to readjust for the fact that we don't draw a WindowBg background
 behind the title bar. This helper function will convert an old TitleBg/TitleBgActive color into a new one with the same visual
 output, given the OLD color and the OLD WindowBg color: ImVec4 ConvertTitleBgCol(const ImVec4& win_bg_col, const ImVec4&
 title_bg_col) { float new_a = 1.0f - ((1.0f - win_bg_col.w) * (1.0f - title_bg_col.w)), k = title_bg_col.w / new_a; return
 ImVec4((win_bg_col.x * win_bg_col.w + title_bg_col.x) * k, (win_bg_col.y * win_bg_col.w + title_bg_col.y) * k, (win_bg_col.z *
 win_bg_col.w + title_bg_col.z) * k, new_a); } If this is confusing, pick the RGB value from title bar from an old screenshot and
 ...
 - 2015/08/29 (1.45) - with the addition of horizontal scrollbar we made various fixes to inconsistencies with dealing with cursor
 position. GetCursorPos()/SetCursorPos() functions now include the scrolled amount. It shouldn't affect the majority of users, but
 take note that SetCursorPosX(100.0f) puts you at +100 from the starting x position which may include scrolling, not at +100 from
 the window left side. GetContentRegionMax()/GetWindowContentRegionMin()/GetWindowContentRegionMax() functions allow include the
 scrolled amount. Typically those were used in cases where no scrolling would happen so it may not be a problem, but watch out!
 ...
 your project.
 - 2015/07/18 (1.44) - fixed angles in ImDrawList::PathArcTo(), PathArcToFast() (introduced in 1.43) being off by an extra PI for
 ...
 - 2015/07/08 (1.43) - switched rendering data to use indexed rendering. this is saving a fair amount of CPU/GPU and enables us to
 get anti-aliasing for a marginal cost. this necessary change will break your rendering function! the fix should be very easy.
 sorry for that :(
                     - if you are using a vanilla copy of one of the imgui_impl_XXXX.cpp provided in the example, you just need to
 update your copy and you can ignore the rest.
 ...
 'UserCallback', 'texture_id' becomes 'TextureId'.
                     - each ImDrawList now contains both a vertex buffer and an index buffer. For each command, render ElemCount/3
 ...
 - 2015/06/14 (1.41) - changed ImageButton() default bg_col parameter from (0,0,0,1) (black) to (0,0,0,0) (transparent) - makes a
 difference when texture have transparence
 ...
 open/closed state of popups. Call OpenPopup() to actually set the "open" state of a popup. BeginPopup() returns true if the popup
 is opened.
 ...
 - 2015/01/19 (1.30) - renamed ImGuiStorage::GetIntPtr()/GetFloatPtr() to GetIntRef()/GetIntRef() because Ptr was conflicting with
 actual pointer storage functions.
 ...
 manage the texture buffer for internal needs. It is now recommended that you sample the font texture with bilinear interpolation.
 - 2015/01/11 (1.30) - added texture identifier in ImDrawCmd passed to your render function (we can now render images). make sure
 to set io.Fonts->TexID.
 ...
 A: - Businesses: please reach out to "contact AT dearimgui.org" if you work in a place using Dear ImGui!
      ...
      This is among the most useful thing you can do for Dear ImGui. With increased funding we can hire more people working on
 this project.
 ...
 help other teams and programmers with taking decisions.
    - If you have issues or if you need to hack into the library, even if you don't expect any support it is useful that you share
 your issues (on github or privately).
...
#        pragma clang diagnostic ignored                                                                                         \
            "-Wunknown-pragmas" // warning : unknown warning group '-Wformat-pedantic *'        // not all warnings are known by
                                // all clang versions.. so ignoring warnings triggers new warnings on some configuration. great!
#        pragma clang diagnostic ignored                                                                                         \
            "-Wold-style-cast" // warning : use of old-style cast                              // yes, they are more terse.
#        pragma clang diagnostic ignored "-Wfloat-equal" // warning : comparing floating point with == or != is unsafe   //
                                                         // storing and comparing against same constants (typically 0.0f) is ok.
#        pragma clang diagnostic ignored                                                                                         \
...
#        pragma clang diagnostic ignored "-Wglobal-constructors" // warning : declaration requires a global destructor // similar
...
static const float WINDOWS_MOUSE_WHEEL_SCROLL_LOCK_TIMER =
    2.00f; // Lock scrolled window (so it doesn't pick child windows that are scrolling through) for a certaint time, unless mouse
           // moved.
...
AddUpdateViewport(ImGuiWindow* window, ImGuiID id, const ImVec2& platform_pos, const ImVec2& size, ImGuiViewportFlags flags);
...
ImGuiStyle::ImGuiStyle()
{
    ...
    DisplayWindowPadding = ImVec2(19, 19); // Window position are clamped to be visible within the display area or monitors by at
    ...
    CurveTessellationTol =
        1.25f; // Tessellation tolerance when using PathBezierCurveTo() without a specific number of segments. Decrease for highly
               // tessellated curves (higher quality, more polygons), increase to reduce quality.
    CircleSegmentMaxError =
        1.60f; // Maximum error (in pixels) allowed when using AddCircle()/AddCircleFilled() or drawing rounded corner rectangles
...
// Scaling the fonts is done separately and is up to you. Important: This operation is lossy because we round all sizes to
// integer. If you need to change your scale multiples, call this over a freshly initialized ImGuiStyle structure rather than
...
ImVec2
ImBezierClosestPoint(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& p, int num_segments)
{
                                            ...
                                            int level)
{
    ...
    else if (level < 10)
    {
        ...
        BezierClosestPointCasteljauStep(p,
                                        p_closest,
                                        p_last,
                                        p_closest_dist2,
                                        x1,
...
// Consider using _stricmp/_strnicmp under Windows or strcasecmp/strncasecmp. We don't actually use either ImStricmp/ImStrnicmp in
// the codebase any more.
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
    ...
    window->DC.PrevLineSize.y =
        (line_height -
         g.Style.ItemSpacing.y); // If we end up needing more accurate data (to e.g. use SameLine) we may as well make the clipper
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 2) // Step 2: dummy step only required if an explicit items_height was passed to constructor or Begin() and user
...
{
    // Set ActiveId even if the _NoMove flag is set. Without it, dragging away from a window with _NoMove would activate hover on
    // other windows.// We _also_ call this when clicking in a window empty space when io.ConfigWindowsMoveFromTitleBarOnly is set, but clear g.MovingWindow afterward. This is because we want ActiveId to be set even when the window is not permitted
    // to move.
    ImGuiContext& g = *GImGui;
    FocusWindow(window);
...
// We use 'undock_floating_node == false' when dragging from title bar to allow moving groups of floating nodes without undocking
// them.
...
void ImGui::StartMouseMovingWindowOrNode(ImGuiWindow* window, ImGuiDockNode* node, bool undock_floating_node)
{
    ...
    {
        // Can undock if:
        // - part of a floating node hierarchy with more than one visible node (if only one is visible, we'll just move the whole
        // hierarchy)
...
void ImGui::UpdateMouseMovingWindowNewFrame()
{
    ...
    if (g.MovingWindow != NULL)
    {
        ...
        // We track it to preserve Focus and so that generally ActiveIdWindow == MovingWindow and ActiveId == MovingWindow->MoveId
        // for consistency.
        ...
        if (g.IO.MouseDown[0] && IsMousePosValid(&g.IO.MousePos))
        {
            ...
            if (moving_window->Pos.x != pos.x || moving_window->Pos.y != pos.y)
            {
                ...
                if (moving_window->ViewportOwned) // Synchronize viewport immediately because some overlays may relies on clipping
    ...
    {
        // When clicking/dragging from a window that has the _NoMove flag, we still set the ActiveId in order to prevent hovering
        // others.
...
void ImGui::UpdateMouseMovingWindowEndFrame()
{
    ...
    // (after we're done with all our widgets, so e.g. clicking on docking tab-bar which have set HoveredId already and not get us
    // here!)
...
static void NewFrameSanityChecks()
{
    ...
    // Perform simple check: error if Docking or Viewport are enabled _exactly_ on frame 1 (instead of frame 0 or later), which is
    // a common error leading to loss of .ini data.
...
{
    // The fonts atlas can be used prior to calling NewFrame(), so we clear it even if g.Initialized is FALSE (which would happen
    // if we never called NewFrame)
    ImGuiContext& g = *context;
    ...
    // Save settings (unless we haven't attempted to load them: CreateContext/DestroyContext without a call to NewFrame shouldn't
    // save an empty file)
        ...
        if (window->RootWindowDockStop->DockIsActive)
            if (window->RootWindow != window->RootWindowDockStop)
                RenderRectFilledWithHole(draw_list,
                                         ...
                                         window->RootWindowDockStop->Rect(),
...
static void FindHoveredWindow()
{
    ...
    // Special handling for the window being moved: Ignore the mouse viewport check (because it may reset/lose its viewport during
    // the undocking frame)
    ...
    for (int i = g.Windows.Size - 1; i >= 0; i--)
    {
        ...
        if (window->HitTestHoleSize.x != 0)
        {
            ...
            ImRect hole_bb((float)(window->HitTestHoleOffset.x),
...
bool ImGui::IsMousePosValid(const ImVec2* mouse_pos)
{
    ...
    // Because GImGui is not dereferenced in every code path, the static analyzer assume that it may be NULL (which it doesn't for
    // other functions).
...
static bool ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    // time).
    // - Not narrowing would mostly benefit the situation where OS windows _without_ decoration have a threshold for hovering when
    ...
    if (clip_with_viewport_rect)
        PushClipRect(window->Viewport->Pos,
                     window->Viewport->Pos + window->Viewport->Size,
                                    ...
                                    float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (flags & ImGuiWindowFlags_MenuBar)
        {
            ...
            menu_bar_rect.ClipWith(window->Rect()); // Soft clipping, in particular child window don't have minimum size covering
        ...
        if (window->DockIsActive && node->IsHiddenTabBar() && !node->IsNoTabBar())
        {
            ...
            // FIXME-DOCK: Ideally we'd use ImGuiCol_TitleBgActive/ImGuiCol_TitleBg here, but neither is guaranteed to be visible
            // enough at this sort of size..
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        if (window->ViewportOwned)
        {
            ...
            // clicking on them won't steal the OS focus away from their parent window (which may be reflected in OS the title bar
            // decoration). Setting _NoFocusOnClick would technically prevent us from bringing back to front in case they are
            // being covered by an OS window from a different app, but it shouldn't be much of a problem considering those are
            // already popups that are closed when clicking elsewhere.
        ...
        // Since 1.71, child window can render their decoration (bg color, border, scrollbars, etc.) within their parent to save a
        // draw call. When using overlapping child windows, this will break the assumption that child z-order is mapped to
        // submission order. We disable this when the parent window has zero vertices, which is a common pattern leading to laying
        // out multiple overlapping child. We also disabled this when we have dimming overlay behind this specific one child.
        ...
        if (p_open != NULL && window->Viewport->PlatformRequestClose && window->Viewport != GetMainViewport())
        {
            if (!window->DockIsActive || window->DockTabIsVisible)
            {
                ...
                g.NavWindowingToggleLayer = false; // Assume user mapped PlatformRequestClose on ALT-F4 so we disable ALT for menu
...
void ImGui::FocusTopMostWindowUnderOne(ImGuiWindow* under_this_window, ImGuiWindow* ignore_window)
{
    ...
    {
        // We may later decide to test for different NoXXXInputs based on the active navigation input (mouse vs nav) but that may
        ...
        if (window != ignore_window && window->WasActive && window->RootWindowDockStop == window)
                ...
                (ImGuiWindowFlags_NoMouseInputs | ImGuiWindowFlags_NoNavInputs))
            {
                ...
                // (essentially the 'most_recently_selected_tab' code in tab bar will do that but on next update) to tell which is
                // the "previous" window. Or we may leverage 'LastFrameFocused/LastFrameJustFocused' and have this function handle
                // child window itself?
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, ImGuiTooltipFlags tooltip_flags)
{
    ...
    if (g.DragDropWithinSourceOrTarget)
    {
        ...
        // PushStyleVar(ImGuiStyleVar_Alpha, g.Style.Alpha * 0.60f); // This would be nice but e.g ColorButton with checkboard has
    ...
    if (tooltip_flags & ImGuiTooltipFlags_OverridePreviousTooltip)
        if (ImGuiWindow* window = FindWindowByName(window_name))
            if (window->Active)
            {
                // Hide previous tooltip from being displayed. We can't easily "reset" the content of a window so we create a new
                // one.
                window->Hidden                   = true;
                window->HiddenFramesCanSkipItems = 1;
                ImFormatString(window_name, IM_ARRAYSIZE(window_name), "##Tooltip_%02d", ++g.TooltipOverrideCount);
...
bool ImGui::BeginPopupModal(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    {
        ImGuiViewportP* viewport = window->WasActive ?
                                       window->Viewport :
...
bool ImGui::OpenPopupOnItemClick(const char* str_id, ImGuiMouseButton mouse_button)
{
    ...
    {
        ImGuiID id =
            str_id ? window->GetID(str_id) : window->DC.LastItemId; // If user hasn't passed an ID, we can use the LastItemID. Using LastItemID as a Popup ID won't conflict!
...
// r_avoid = the rectangle to avoid (e.g. for tooltip it is a rectangle around the mouse cursor which we want to avoid. for popups
// it's a small point around the cursor.) r_outer = the visible area rectangle, minus safe area padding. If our popup size won't
// fit because of safe area padding we ignore it. (r_outer is usually equivalent to the viewport rectangle minus padding, but when
...
static void ImGui::UpdateViewportsNewFrame()
{
    ...
    for (int n = 0; n < g.Viewports.Size; n++)
    {
        ...
        if (viewport->DpiScale != 0.0f && new_dpi_scale != viewport->DpiScale)
        {
            ...
            // border. (Minor: since our sizes do not perfectly linearly scale, deferring the click offset scale until we know the
            // actual window scale ratio may get us slightly more precise mouse positioning.)
    ...
    else
    {
        ...
        // A) won't take account of the possibility that non-imgui windows may be in-between our dragged window and our target
        // window. B) uses LastFrameAsRefViewport as a flawed replacement for the last time a window was focused (we could/should
        // fix that by introducing Focus functions in PlatformIO)
        viewport_hovered = FindHoveredViewportFromPlatformWindowStack(g.IO.MousePos);
    }
    ...
    // - consider the case of holding on a menu item to browse child menus: even thou a mouse button is held, there's no active id
    // because menu items only react on mouse release.
...
static void ImGui::UpdateViewportsEndFrame()
{
    ...
    g.Viewports[0]
        ->ClearRequestFlags(); // Clear main viewport flags because UpdatePlatformWindows() won't do it and may not even be called
}

// FIXME: We should ideally refactor the system to call this every frame (we currently don't)
ImGuiViewportP*
ImGui::AddUpdateViewport(ImGuiWindow* window, ImGuiID id, const ImVec2& pos, const ImVec2& size, ImGuiViewportFlags flags)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(id != 0);
...
// Custom renderers may prefer to not call this function at all, and instead iterate the publicly exposed platform data and handle
// rendering/sync themselves. The Render/Swap functions stored in ImGuiPlatformIO are merely here to allow for this helper to
// exist, but you can do it yourself:
//
...
static void
DockContextClearNodes(ImGuiContext* ctx, ImGuiID root_id, bool clear_persistent_docking_refs); // Use root_id==0 to clear all
static void
DockContextBuildNodesFromSettings(ImGuiContext* ctx, ImGuiDockNodeSettings* node_settings_array, int node_settings_count);
...
void ImGui::DockContextUpdateDocking(ImGuiContext* ctx)
{
    ...
    // We can have NULL pointers when we delete nodes, but because ID are recycled this should amortize nicely (and our node count
...
static void
ImGui::DockContextBuildNodesFromSettings(ImGuiContext* ctx, ImGuiDockNodeSettings* node_settings_array, int node_settings_count)
{
    // Build nodes
...
void ImGui::DockContextProcessDock(ImGuiContext* ctx, ImGuiDockRequest* req)
{
    ...
    {
        // Create tab bar before we call DockNodeMoveWindows (which would attempt to move the old tab-bar, which would lead us to
        // payload tabs wrongly appearing before target tabs!)
...
static void ImGui::DockNodeAddWindow(ImGuiDockNode* node, ImGuiWindow* window, bool add_to_tab_bar)
{
    ...
    // If more than 2 windows appeared on the same frame, we'll create a new hosting DockNode from the point of the second window
    // submission. Then we need to hide the first window (after its been output) otherwise it would be visible as a standalone
    ...
    // storage. In particular it is important we init the viewport from the first window so we don't create two viewports and drop
    // one.
...
static void ImGui::DockNodeRemoveWindow(ImGuiDockNode* node, ImGuiWindow* window, ImGuiID save_dock_id)
{
    ...
    // Update visibility immediately is required so the DockNodeUpdateRemoveInactiveChilds() processing can reflect changes up the
    // tree
    DockNodeUpdateVisibleFlag(node);
}
...
static void ImGui::DockNodeUpdateForRootNode(ImGuiDockNode* node)
{
    ...
    // When node has mixed windows, prioritize the class with the most constraint (DockingAllowUnclassed = false) as the reference
    // to copy.
...
static void ImGui::DockNodeUpdate(ImGuiDockNode* node)
{
    ...
    // code. It would require a generalization of CalcWindowExpectedSize(), probably extracting code away from Begin(). In reality
    // it isn't very important as user quickly ends up with size data in .ini file.
    ...
    // know the correct pos/size after processing the resizing splitters. So we are using the DrawList channel splitting facility
    // to submit drawing primitives out of order!
...
// Compare TabItem nodes given the last known DockOrder (will persist in .ini file as hint), used to sort tabs when multiple tabs
// are added on the same frame.
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode* node, ImGuiWindow* host_window)
{
    ...
    // This is to facilitate computing those ID from the outside, and will affect more or less only the ID of the collapse button,
    // popup and tabs, as docked windows themselves will override the stack with their own root ID.
    ...
    if (is_focused)
        node->LastFrameFocused = g.FrameCount;
    ImU32 title_bar_col =
        GetColorU32(host_window->Collapsed ? ImGuiCol_TitleBgCollapsed : is_focused ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg);
                                            ...
                                            ImVec2* out_window_menu_button_pos)
{
    ...
    {
        r.Max.x -=
            g.FontSize; // +1.0f; // In DockNodeUpdateTabBar() we currently display a disabled close button even if there is none.
    }
    if (node->HasWindowMenuButton && g.Style.WindowMenuButtonPosition == ImGuiDir_Left)
    {
        r.Min.x += g.FontSize; // + g.Style.ItemInnerSpacing.x; // <-- Adding ItemInnerSpacing makes the title text moves slightly
                               // when in a docking tab bar. Instead we adjusted RenderArrowDockMenu()
    }
                                                 ...
                                                 ImVec2* test_mouse_pos)
{
    ...
    if (outer_docking)
    {
        ...
        // off = ImVec2(ImFloor(parent.GetWidth() * 0.5f - GetFrameHeightWithSpacing() * 1.4f - hs_h), ImFloor(parent.GetHeight()
    ...
    if (data->IsDropAllowed)
    {
        ...
        if (data->SplitDir != ImGuiDir_None || data->IsCenterAvailable)
            for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++)
                overlay_draw_lists[overlay_n]->AddRectFilled(overlay_rect.Min,
                                                             overlay_rect.Max,
                                                             overlay_col_main,
                                                             host_window->WindowRounding);
    }
        ...
        else
        {
            ...
            if (g.ActiveId == splitter_id)
            {
                ...
                node->ChildNodes[n]->Pos.y + node->ChildNodes[n]->Size.y), IM_COL32(255, 0, 255, 255), 3.0f); else
                        draw_list->AddLine(ImVec2(node->ChildNodes[n]->Pos.x, resize_limits[n]), ImVec2(node->ChildNodes[n]->Pos.x
                + node->ChildNodes[n]->Size.x, resize_limits[n]), IM_COL32(255, 0, 255, 255), 3.0f);
                */
            }
...
void ImGui::DockSpace(ImGuiID id, const ImVec2& size_arg, ImGuiDockNodeFlags flags, const ImGuiWindowClass* window_class)
{
    ...
    // It is possible that the node has already been claimed by a docked window which appeared before the DockSpace() node, so we
    // overwrite IsDockSpace again.
    ...
    window_flags |=
        ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar;
...
// can't Begin/End in a single function. So if you want a menu bar you need to repeat this code manually ourselves. As with
// advanced other Docking API, we may change this function signature.
ImGuiID
ImGui::DockSpaceOverViewport(ImGuiViewport* viewport, ImGuiDockNodeFlags dockspace_flags, const ImGuiWindowClass* window_class)
{
    if (viewport == NULL)
                                    ...
                                    ImGuiID* out_id_at_opposite_dir)
{
    ...
    req.DockSplitDir     = split_dir;
    req.DockSplitRatio   = ImSaturate((split_dir == ImGuiDir_Left || split_dir == ImGuiDir_Up) ? size_ratio_for_node_at_dir :
                                     ...
                                     ImVector<const char*>* in_window_remap_pairs)
{
    ...
    // Anything else in the source nodes of 'node_remap_pairs' are windows that were docked in src_dockspace_id but are not owned
    // by it (unaffiliated windows, e.g. "ImGui Demo") Find those windows and move to them to the cloned dock node. This may be
    // optional?
    for (int dock_remap_n = 0; dock_remap_n < node_remap_pairs.Size; dock_remap_n += 2)
...
static ImGuiDockNode* ImGui::DockContextBindNodeToWindow(ImGuiContext* ctx, ImGuiWindow* window)
{
    ...
    // If the node just turned visible and is part of a hierarchy, it doesn't have a Size assigned by DockNodeTreeUpdatePosSize()
    // yet, so we're forcing a Pos/Size update from the first ancestor that is already visible (often it will be the root node).
...
#    endif
    ...
    if (node->IsHiddenTabBar() || node->IsNoTabBar())
        ...
        window->Flags &=
            ~ImGuiWindowFlags_NoTitleBar; // Clear the NoTitleBar flag in case the user set it: confusingly enough we need a title
                                          // bar height so we are correctly offset, but it won't be displayed!
...
void ImGui::ShowViewportThumbnails()
{
    ...
    // We don't display full monitor bounds (we could, but it often looks awkward), instead we display just enough to cover all of
    // our viewports.
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        static ImRect GetTableRect(ImGuiTable* table, int rect_type, int n)
        {
            ...
            else if (rect_type == TRT_ColumnsContentHeadersUsed)
            {
                ...
                return ImRect(c->MinX,
                              ...
                              c->MinX + c->ContentWidthHeadersUsed,
                              table->InnerClipRect.Min.y + table->LastFirstRowHeight);
            } // Note: y1/y2 not always accurate
            else if (rect_type == TRT_ColumnsContentHeadersDesired)
            {
                ImGuiTableColumn* c = &table->Columns[n];
                return ImRect(c->MinX,
                              table->InnerClipRect.Min.y,
                              c->MinX + c->ContentWidthHeadersDesired,
                              table->InnerClipRect.Min.y + table->LastFirstRowHeight);
            } // "
            else if (rect_type == TRT_ColumnsContentRowsFrozen)
            {
                ImGuiTableColumn* c = &table->Columns[n];
                return ImRect(c->MinX,
                              table->InnerClipRect.Min.y,
                              c->MinX + c->ContentWidthRowsFrozen,
                              table->InnerClipRect.Min.y + table->LastFirstRowHeight);
            } // "
            else if (rect_type == TRT_ColumnsContentRowsUnfrozen)
            {
                ...
                return ImRect(c->MinX,
                              table->InnerClipRect.Min.y + table->LastFirstRowHeight,
                              c->MinX + c->ContentWidthRowsUnfrozen,
        ...
        static void NodeWindow(ImGuiWindow* window, const char* label)
        {
            ...
            bool open =
                ImGui::TreeNode(label, "%s '%s', %d @ 0x%p", label, window->Name, (window->Active || window->WasActive), window);
            ...
            ImGui::BulletText("DockId: 0x%04X, DockOrder: %d, Act: %d, Vis: %d",
                              window->DockId,
                              window->DockOrder,
                              window->DockIsActive,
        ...
        static void NodeViewport(ImGuiViewportP* viewport)
        {
                                ...
                                viewport->Window ? viewport->Window->Name : "N/A"))
            {
                ...
                ImGui::BulletText("Pos: (%.0f,%.0f), Size: (%.0f,%.0f), Monitor: %d, DpiScale: %.0f%%",
                                  viewport->Pos.x,
                                  viewport->Pos.y,
                                  viewport->Size.x,
                                  viewport->Size.y,
                ...
                ImGui::BulletText("Flags: 0x%04X =%s%s%s%s%s%s%s",
                                  viewport->Flags,
        ...
        static void NodeDockNode(ImGuiDockNode* node, const char* label)
        {
            ...
            bool open;
            if (node->Windows.Size > 0)
                open = ImGui::TreeNode((void*)(intptr_t)node->ID,
            ...
            if (open)
            {
                ...
                ImGui::BulletText("SelectedTabID: 0x%08X, LastFocusedNodeID: 0x%08X",
                                  node->SelectedTabId,
                                  node->LastFocusedNodeId);
                ImGui::BulletText("Misc:%s%s%s%s",
                ...
                if (ImGui::TreeNode("flags", "LocalFlags: 0x%04X SharedFlags: 0x%04X", node->LocalFlags, node->SharedFlags))
                {
                    ...
                    ImGui::CheckboxFlags("LocalFlags: NoWindowMenuButton",
                                         (ImU32*)&node->LocalFlags,
                                         ImGuiDockNodeFlags_NoWindowMenuButton);
                    ImGui::CheckboxFlags("LocalFlags: NoCloseButton",
                                         (ImU32*)&node->LocalFlags,
        ...
        static void NodeStorage(ImGuiStorage* storage, const char* label)
        {
            ...
            for (int n = 0; n < storage->Data.Size; n++)
            {
                ...
                ImGui::BulletText("Key 0x%08X Value { i: %d }",
                                  p.key,
        ...
        static void NodeTable(ImGuiTable* table)
        {
            ...
            ImFormatString(p,
                           ...
                           (table->LastFrameActive < ImGui::GetFrameCount() - 2) ? " *Inactive*" : "");
            ...
            if (open)
            {
                for (int n = 0; n < table->ColumnsCount; n++)
                {
                    ...
                    ImGui::BulletText("Column %d order %d name '%s': +%.1f to +%.1f\n"
                                      ...
                                      column->IsActive,
                                      column->IsClipped,
                                      column->DrawChannelRowsBeforeFreeze,
                                      column->DrawChannelRowsAfterFreeze,
                                      column->WidthGiven,
                                      column->WidthRequested,
                                      column->ResizeWeight,
                                      column->ContentWidthRowsFrozen,
                                      column->ContentWidthRowsUnfrozen,
                                      column->ContentWidthHeadersUsed,
                                      column->ContentWidthHeadersDesired,
                                      column->SortOrder,
                                      (column->SortDirection == ImGuiSortDirection_Ascending) ?
                                          "Ascending" :
                                          (column->SortDirection == ImGuiSortDirection_Descending) ? "Descending" : "None",
                                      column->UserID,
                                      column->Flags,
                ...
                if (settings && ImGui::TreeNode("Settings"))
                {
                    ...
                    for (int n = 0; n < settings->ColumnsCount; n++)
                    {
                        ...
                        ImGuiSortDirection sort_dir =
                            (column_settings->SortOrder != -1) ? column_settings->SortDirection : ImGuiSortDirection_None;
    ...
    if (ImGui::TreeNode("Docking"))
    {
        ...
        if (ImGui::TreeNode("Settings"))
        {
            ...
            for (int n = 0; n < dc->SettingsNodes.Size; n++)
            {
                ...
                ImGui::BulletText("Node %08X, Parent %08X, SelectedTab %08X ('%s')",
                                  settings->ID,
...
#        endif // #define IMGUI_HAS_DOCK
    ...
    if (ImGui::TreeNode("Internal state"))
    {
        ...
        ImGui::Text("HoveredId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d",
                    ...
                    g.HoveredIdTimer,
                    g.HoveredIdAllowOverlap); // Data is "in-flight" so depending on when the Metrics window is called we may see
    ...
    if (ImGui::TreeNode("Tools"))
    {
        ...
        ImGui::SetNextItemWidth(ImGui::GetFontSize() * 12);
        show_tables_rects |=
            ImGui::Combo("##show_table_rects_type", &show_tables_rect_type, trt_rects_names, TRT_Count, TRT_Count);
        if (show_tables_rects && g.NavWindow)
        {
            for (int table_n = 0; table_n < g.Tables.GetSize(); table_n++)
            {
                ...
                if (ImGui::IsItemHovered())
                    ImGui::GetForegroundDrawList()->AddRect(table->OuterRect.Min - ImVec2(1, 1),
                ...
                for (int rect_n = 0; rect_n < TRT_Count; rect_n++)
                {
                    if (rect_n >= TRT_ColumnsRect)
                    {
                        ...
                        for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
                        {
                            ...
                            ImGui::Text("(%6.1f,%6.1f) (%6.1f,%6.1f) Size (%6.1f,%6.1f) Col %d %s",
                                        r.Min.x,
                                        r.Min.y,
                                        r.Max.x,
                                        r.Max.y,
                            ...
                            if (ImGui::IsItemHovered())
                                ImGui::GetForegroundDrawList()->AddRect(r.Min - ImVec2(1, 1),
                                                                        r.Max + ImVec2(1, 1),
                                                                        IM_COL32(255, 255, 0, 255),
                                                                        0.0f,
                                                                        ~0,
