    ...
    if (ImGui::CollapsingHeader("Configuration"))
    {
        ...
        if (ImGui::TreeNode("Configuration##2"))
        {
            ...
            if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
            {
                ...
                ImGui::SameLine();
                HelpMarker("Toggling this at runtime is normally unsupported (most platform back-ends won't refresh the task bar "
                           "icon state right away).");
                ImGui::Checkbox("io.ConfigViewportsNoDecoration", &io.ConfigViewportsNoDecoration);
                ImGui::SameLine();
                HelpMarker("Toggling this at runtime is normally unsupported (most platform back-ends won't refresh the "
...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Text Input"))
    {
        if (ImGui::TreeNode("Multi-line Text Input"))
        {
            ...
            HelpMarker("You can use the ImGuiInputTextFlags_CallbackResize facility if you need to wire InputTextMultiline() to "
                       "a dynamic string type. See misc/cpp/imgui_stdlib.h for an example. (This is not demonstrated in "
    ...
    if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)"))
    {
        ...
        // This is useful in particular if you want to create a context menu (with BeginPopupContextItem) associated to the title
        // bar of a window. This will also work when docked into a Tab (the Tab replace the Title Bar and guarantee the same
        // properties).
        ...
        if (test_window)
        {
            ...
            // stuck. Could we fix this through an ImGuiWindowClass feature? Or an API call to tag our parent as "don't skip
            // items"?
...
static void ShowDemoWindowTables()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
        ...
        // Another subtle variant, we call TableNextCell() _before_ each cell. At the end of a row, TableNextCell() will create a
        // new row. Note that we don't call TableNextRow() here! If we want to call TableNextRow(), then we don't need to call
        // TableNextCell() for the first cell.
        HelpMarker("Only using TableNextCell(), which tends to be convenient for tables where every cells contains the same type "
    ...
    if (ImGui::TreeNode("Resizable, fixed"))
    {
        ...
        HelpMarker("Using _Resizable + _SizingPolicyFixedX flags.\nFixed-width columns generally makes more sense if you want to "
                   "use horizontal scrolling.");
        static ImGuiTableFlags flags = ImGuiTableFlags_Resizable | ImGuiTableFlags_SizingPolicyFixedX |
                                       ImGuiTableFlags_BordersOuter | ImGuiTableFlags_BordersV;
        // ImGui::CheckboxFlags("ImGuiTableFlags_ScrollX", (unsigned int*)&flags, ImGuiTableFlags_ScrollX); // FIXME-TABLE:
        // Explain or fix the effect of enable Scroll on outer_size
        if (ImGui::BeginTable("##table1", 3, flags))
        {
            for (int row = 0; row < 5; row++)
            {
    ...
    if (ImGui::TreeNode("Resizable, mixed"))
    {
        ...
        static ImGuiTableFlags flags = ImGuiTableFlags_SizingPolicyFixedX | ImGuiTableFlags_RowBg | ImGuiTableFlags_Borders |
    ...
    if (ImGui::TreeNode("Reorderable, hideable, with headers"))
    {
        ...
        if (ImGui::BeginTable("##table1", 3, flags))
        {
            // Submit columns name with TableSetupColumn() and call TableAutoHeaders() to create a row with a header in each
    ...
    if (ImGui::TreeNode("Vertical scrolling, with clipping"))
    {
        ...
        static ImGuiTableFlags flags = ImGuiTableFlags_ScrollY | ImGuiTableFlags_ScrollFreezeTopRow | ImGuiTableFlags_RowBg |
    ...
    {
        HelpMarker("When ScrollX is enabled, the default sizing policy becomes ImGuiTableFlags_SizingPolicyFixedX, as "
                   "automatically stretching columns doesn't make much sense with horizontal scrolling.\n\nAlso note that as of "
                   "the current version, you will almost always want to enable ScrollY along with ScrollX, because the container "
                   "window won't automatically extend vertically to fix contents (this may be improved in future versions).");
        ...
        static ImGuiTableFlags flags = ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY | ImGuiTableFlags_ScrollFreezeTopRow |
                                       ImGuiTableFlags_ScrollFreezeLeftColumn | ImGuiTableFlags_RowBg |
                                       ImGuiTableFlags_BordersOuter | ImGuiTableFlags_BordersV | ImGuiTableFlags_Resizable |
                                       ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable;
        ImGui::CheckboxFlags("ImGuiTableFlags_ScrollY", (unsigned int*)&flags, ImGuiTableFlags_ScrollY);
        ImGui::CheckboxFlags("ImGuiTableFlags_ScrollFreezeTopRow", (unsigned int*)&flags, ImGuiTableFlags_ScrollFreezeTopRow);
        ...
        if (ImGui::BeginTable("##table1", 7, flags, size))
        {
            ImGui::TableSetupColumn("Line #", ImGuiTableColumnFlags_NoHide); // Make the first column not hideable to match our
                                                                             // use of ImGuiTableFlags_ScrollFreezeLeftColumn
            ImGui::TableSetupColumn("One", ImGuiTableColumnFlags_None);
            ImGui::TableSetupColumn("Two", ImGuiTableColumnFlags_None);
            ImGui::TableSetupColumn("Three", ImGuiTableColumnFlags_None);
            ...
            for (int row = 0; row < 20; row++)
            {
                ...
                {
                    // Both TableNextCell() and TableSetColumnIndex() return false when a column is not visible, which can be used
                    // for clipping.
    ...
    if (ImGui::TreeNode("Columns flags"))
    {
        ...
        if (ImGui::BeginTable("##flags", column_count, ImGuiTableFlags_None))
        {
            for (int column = 0; column < column_count; column++)
            {
                ...
                ImGui::CheckboxFlags("_NoSortAscending",
                                     (unsigned int*)&column_flags[column],
                                     ImGuiTableColumnFlags_NoSortAscending);
                ImGui::CheckboxFlags("_NoSortDescending",
                                     (unsigned int*)&column_flags[column],
        ...
        const ImGuiTableFlags flags = ImGuiTableFlags_SizingPolicyFixedX | ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersOuter |
    ...
    {
        HelpMarker("This section allows you to interact and see the effect of StretchX vs FixedX sizing policies depending on "
        ...
        ImGui::CheckboxFlags("ImGuiTableFlags_ScrollY", (unsigned int*)&flags, ImGuiTableFlags_ScrollY);
        if (ImGui::CheckboxFlags("ImGuiTableFlags_SizingPolicyStretchX",
                                 (unsigned int*)&flags,
                                 ImGuiTableFlags_SizingPolicyStretchX))
            flags &= ~(ImGuiTableFlags_SizingPolicyMaskX_ ^
                       ImGuiTableFlags_SizingPolicyStretchX); // Can't specify both sizing polices so we clear the other
        ImGui::SameLine();
        HelpMarker("Default if _ScrollX if disabled.");
        if (ImGui::CheckboxFlags("ImGuiTableFlags_SizingPolicyFixedX", (unsigned int*)&flags, ImGuiTableFlags_SizingPolicyFixedX))
            flags &= ~(ImGuiTableFlags_SizingPolicyMaskX_ ^
                       ImGuiTableFlags_SizingPolicyFixedX); // Can't specify both sizing polices so we clear the other
        ImGui::SameLine();
        HelpMarker("Default if _ScrollX if enabled.");
        ImGui::CheckboxFlags("ImGuiTableFlags_Resizable", (unsigned int*)&flags, ImGuiTableFlags_Resizable);
    ...
    if (ImGui::TreeNode("Tree view"))
    {
        static ImGuiTableFlags flags =
            ImGuiTableFlags_BordersV | ImGuiTableFlags_BordersHOuter | ImGuiTableFlags_Resizable | ImGuiTableFlags_RowBg;
        // ImGui::CheckboxFlags("ImGuiTableFlags_Scroll", (unsigned int*)&flags, ImGuiTableFlags_Scroll);
        // ImGui::CheckboxFlags("ImGuiTableFlags_ScrollFreezeLeftColumn", (unsigned int*)&flags,
        // ImGuiTableFlags_ScrollFreezeLeftColumn);
    ...
    // FIXME-TABLE: Currently this doesn't get us feature-parity with TableAutoHeaders(), e.g. missing context menu.  Tables API
    // needs some work!
    if (open_action != -1)
        ImGui::SetNextItemOpen(open_action != 0);
    ...
    if (ImGui::TreeNode("Sorting"))
    {
        ...
        static ImGuiTableFlags flags = ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable |
                                       ...
                                       ImGuiTableFlags_BordersV | ImGuiTableFlags_ScrollY | ImGuiTableFlags_ScrollFreezeTopRow;
    ...
    {
        static ImGuiTableFlags flags = ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable |
                                       ImGuiTableFlags_MultiSortable | ImGuiTableFlags_RowBg | ImGuiTableFlags_Borders |
                                       ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY | ImGuiTableFlags_ScrollFreezeTopRow |
                                       ImGuiTableFlags_ScrollFreezeLeftColumn | ImGuiTableFlags_SizingPolicyFixedX;
        ...
        if (ImGui::TreeNodeEx("Options"))
        {
            ...
            if (ImGui::CheckboxFlags("ImGuiTableFlags_SizingPolicyStretchX",
                ...
                flags &= ~(ImGuiTableFlags_SizingPolicyMaskX_ ^
                           ImGuiTableFlags_SizingPolicyStretchX); // Can't specify both sizing polices so we clear the other
            ImGui::SameLine();
            HelpMarker("[Default if ScrollX is off]\nFit all columns within available width (or specified inner_width). Fixed "
                       "and Stretch columns allowed.");
            if (ImGui::CheckboxFlags("ImGuiTableFlags_SizingPolicyFixedX",
                                     (unsigned int*)&flags,
                                     ImGuiTableFlags_SizingPolicyFixedX))
                flags &= ~(ImGuiTableFlags_SizingPolicyMaskX_ ^
                           ImGuiTableFlags_SizingPolicyFixedX); // Can't specify both sizing polices so we clear the other
            ImGui::SameLine();
            HelpMarker("[Default if ScrollX is on]\nEnlarge as needed: enable scrollbar if ScrollX is enabled, otherwise extend "
                       "parent window's contents rectangle. Only Fixed columns allowed. Stretched columns will calculate their "
            ...
            // For the purpose of our "advanced" demo, we expose the 3 freezing variants on both axises instead of only exposing
            // the most common flag.
            // ImGui::CheckboxFlags("ImGuiTableFlags_ScrollFreezeTopRow", (unsigned int*)&flags,
            // ImGuiTableFlags_ScrollFreezeTopRow); ImGui::CheckboxFlags("ImGuiTableFlags_ScrollFreezeLeftColumn", (unsigned
            ...
            int freeze_col_count =
                (flags & ImGuiTableFlags_ScrollFreezeColumnsMask_) >> ImGuiTableFlags_ScrollFreezeColumnsShift_;
            ...
            if (ImGui::DragInt("ImGuiTableFlags_ScrollFreezeTopRow/2Rows/3Rows", &freeze_row_count, 0.2f, 0, 3))
                if (freeze_row_count >= 0 && freeze_row_count <= 3)
                    flags = (flags & ~ImGuiTableFlags_ScrollFreezeRowsMask_) |
            ...
            if (ImGui::DragInt("ImGuiTableFlags_ScrollFreezeLeftColumn/2Columns/3Columns", &freeze_col_count, 0.2f, 0, 3))
                if (freeze_col_count >= 0 && freeze_col_count <= 3)
                    flags = (flags & ~ImGuiTableFlags_ScrollFreezeColumnsMask_) |
                            (freeze_col_count << ImGuiTableFlags_ScrollFreezeColumnsShift_);
            ...
            ImGui::SameLine();
            HelpMarker("If scrolling is disabled (ScrollX and ScrollY not set), the table is output directly in the parent "
                       "window. OuterSize.y then becomes the minimum size for the table, which will extend vertically if there "
                       "are more rows (unless NoHostExtendV is set).");

            // From a user point of view we will tend to use 'inner_width' differently depending on whether our table is embedding
            // scrolling. To facilitate experimentation we expose two values and will select the right one depending on active
            // flags.
        ...
        if (ImGui::BeginTable("##table", 5, flags, outer_size_enabled ? outer_size_value : ImVec2(0, 0), inner_width_to_use))
        {
            ...
            ImGui::TableSetupColumn("ID",
                                    ImGuiTableColumnFlags_DefaultSort | ImGuiTableColumnFlags_WidthFixed |
                                        (lock_first_column_visibility ? ImGuiTableColumnFlags_NoHide : 0),
                                    -1.0f,
                                    MyItemColumnID_ID);
            ImGui::TableSetupColumn("Name", ImGuiTableColumnFlags_WidthFixed, -1.0f, MyItemColumnID_Name);
            ImGui::TableSetupColumn("Action",
            ...
            ImGui::TableSetupColumn(
                ...
                ImGuiTableColumnFlags_PreferSortDescending | ImGuiTableColumnFlags_WidthStretch,
                1.0f,
                MyItemColumnID_Quantity); // , ImGuiTableColumnFlags_None | ImGuiTableColumnFlags_WidthAlwaysAutoResize);
            ImGui::TableSetupColumn("Description",
                                    ImGuiTableColumnFlags_WidthStretch,
                                    1.0f,
                                    MyItemColumnID_Description); // , ImGuiTableColumnFlags_WidthAlwaysAutoResize);
...
static void ShowExampleAppSimpleOverlay(bool* p_open)
{
    ...
    if (corner != -1)
    {
        ...
        ImVec2 window_pos = ImVec2((corner & 1) ? (viewport->Pos.x + viewport->Size.x - DISTANCE) : (viewport->Pos.x + DISTANCE),
    ...
    if (ImGui::Begin("Example: Simple overlay",
                     p_open,
                     (corner != -1 ? ImGuiWindowFlags_NoMove : 0) | ImGuiWindowFlags_NoDocking | ImGuiWindowFlags_NoTitleBar |
...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    if (ImGui::BeginTabBar("##TabBar"))
    {
        ...
        if (ImGui::BeginTabItem("Primitives"))
        {
            ...
            ImGui::SameLine();
            if (ImGui::RadioButton("16", gradient_steps == 16))
            {
                gradient_steps = 16;
            }
            ImGui::SameLine();
            if (ImGui::RadioButton("32", gradient_steps == 32))
            {
                gradient_steps = 32;
            }
            ImGui::SameLine();
            if (ImGui::RadioButton("256", gradient_steps == 256))
            {
                gradient_steps = 256;
            }
            ...
            for (int n = 0; n < gradient_steps; n++)
            {
                ...
                draw_list->AddRectFilled(ImVec2(x + gradient_size.x * f0, y),
                                         ImVec2(x + gradient_size.x * f1, y + gradient_size.y),
                                         col32);
            }
...
void ShowExampleAppDockSpace(bool* p_open)
{
    ...
    if (ImGui::BeginMenuBar())
    {
        ...
        HelpMarker(
            ...
            "_within_ another window. This is useful so you can decorate your main application window (e.g. with a menu bar)."
            "\n\n"
            "ImGui::DockSpace() comes with one hard constraint: it needs to be submitted _before_ any window which may be docked "
            "into it. Therefore, if you use a dock spot as the central point of your application, you'll probably want it to be "
            "part of the very first window you are submitting to imgui every frame."
            "\n\n"
            "(NB: because of this constraint, the implicit \"Debug\" window can not be docked into an explicit DockSpace() node, "
            "because that window is submitted as part of the NewFrame() call. An easy workaround is that you can create your own "
            "implicit \"Debug##2\" window after calling DockSpace() and leave it in the window stack for anyone to use.)");
