...
static float rescale(float t, float min, float max, PlotConfig::Scale::Type type)
{
    switch (type)
    {
        case PlotConfig::Scale::Linear: return t;
        case PlotConfig::Scale::Log10: return log10(ImLerp(min, max, t) / min) / log10(max / min);
    }
    return 0;
}
...
static float rescale_inv(float t, float min, float max, PlotConfig::Scale::Type type)
{
    ...
    {
        case PlotConfig::Scale::Linear: return t;
        case PlotConfig::Scale::Log10: return (pow(max / min, t) * min - min) / (max - min);
    }
    return 0;
}
...
PlotStatus Plot(const char* label, const PlotConfig& conf)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + conf.frame_size);
    ...
    if (conf.values.count > 0)
    {
            ...
            switch (conf.scale.type)
            {
                case PlotConfig::Scale::Linear:
                {
                    ...
                    for (int i = 0; i <= cnt; ++i)
                    {
                        ...
                        window->DrawList->AddLine(ImVec2(x0, y0),
                                                  ImVec2(x0, y1),
        ...
        for (int i = 0; i < ys_count; ++i)
        {
            ...
            for (int n = 0; n < res_w; n++)
            {
                ...
                const ImVec2 tp1 =
                    ImVec2(rescale(t1, x_min, x_max, conf.scale.type), 1.0f - ImSaturate((v1 - conf.scale.min) * inv_scale));
        ...
        if (conf.selection.show)
        {
            ...
            ImVec2 pos0          = ImLerp(inner_bb.Min, inner_bb.Max, ImVec2(fSelectionStep * *conf.selection.start, 0.f));
