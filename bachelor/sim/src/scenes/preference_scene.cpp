...
void PreferenceScene::show_keybinding_ui()
{
    ...
    for (auto&& [context, action_h] : m_display_map)
    {
        ...
        ImGui::TableSetupColumn(m_context->preferences->get_string("key_press").data());
        ImGui::TableSetupColumn(m_context->preferences->get_string("key_action").data());
        ...
        display_select_action_ui(context);
        display_change_bindings_ui(context);
...
void PreferenceScene::display_select_action_ui(input::EKeyContext context)
{
    if (ImGui::Button(m_context->preferences->get_string("key_new_binding").data()))
    {
...
void PreferenceScene::display_change_bindings_ui(input::EKeyContext context)
{
                               ...
                               ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove))
    {
        ...
        if (n_key != SDL_SCANCODE_UNKNOWN)
        {
            ...
            if (m_bind_index >= 0 && m_bind_index < m_display_map[context].size())
            {
                ...
                auto&& it = std::find_if(m_display_map[context].begin(), m_display_map[context].end(), [n_key](const auto& p) {
                    return p.first == SDL_GetScancodeName(n_key);
                });
        ...
        else if (bind_mouse != input::EMouse::BtnNone)
        {
            ...
            if (m_bind_index >= 0 && m_bind_index < m_display_map[context].size())
            {
                ...
                auto&& it = std::find_if(m_display_map[context].begin(),
                                         ...
                                         [bind_mouse](const auto& p) { return p.first == input::mouse_to_string(bind_mouse); });
