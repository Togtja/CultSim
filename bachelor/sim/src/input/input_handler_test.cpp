...
TEST_CASE("attempting to create a key and use it")
{
    ...
    bool success = false;
    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_A, Action::MoveLeft, [&success]() { success = true; });
...
TEST_CASE("attempting to use keybinding after clear")
{
    ...
    auto& input = get_input();
    int times   = 0;
    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F12, Action::ZoomOut, [&times]() { times++; });
    input.handle_input(e);
    input.handle_input(e);
    CHECK(times == 2);
    input.clear();
...
TEST_CASE("attempting to use keybinding after unbind")
{
    ...
    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F11, Action::MoveLeft, [&times1]() { times1++; });
    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F10, Action::ZoomIn, [&times0]() { times0++; });
...
TEST_CASE("attempting to bind same key to different function")
{
    ...
    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F19, Action::MoveLeft, [&times1]() { times1++; });
    input.handle_input(e);
    CHECK(times1 == 1);

    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F19, Action::MoveLeft, [&times0]() { times0++; });
    input.handle_input(e);
    CHECK(times1 == 1);
    CHECK(times0 == 1);
    input.clear();
...
TEST_CASE("attempting to bind diffrent key to same action")
{
    ...
    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F19, Action::MoveLeft, [&times]() { times++; });
    input.handle_input(f19);
    CHECK(times == 1);

    input.bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F18, Action::MoveLeft);
...
TEST_CASE("attempting to add default context on top of something else")
{
    ...
    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F13, Action::MoveLeft, [&times1]() { times1++; });
    input.fast_bind_key(EKeyContext::Agent, SDL_SCANCODE_F13, Action::MoveLeft, [&times0]() { times0++; });
    input.handle_input(e);
    CHECK(times1 == 1);
    CHECK(times0 == 0);
...
TEST_CASE("attempting to add non default context on top of another one")
{
    ...
    input.fast_bind_key(EKeyContext::Agent, SDL_SCANCODE_F13, Action::MoveLeft, [&times1]() { times1++; });
    input.fast_bind_key(EKeyContext::AgentOnHover, SDL_SCANCODE_F13, Action::MoveLeft, [&times0]() { times0++; });
...
TEST_CASE("attempting to remove default context from stack")
{
    ...
    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F24, Action::MoveLeft, [&times]() { times++; });
    input.handle_input(e);
    CHECK(times == 1);
...
TEST_CASE("attempting to remove non default context from stack by context")
{
    ...
    input.fast_bind_key(EKeyContext::Agent, SDL_SCANCODE_F11, Action::MoveLeft, [&times1]() { times1++; });
    input.fast_bind_key(EKeyContext::AgentOnHover, SDL_SCANCODE_F13, Action::MoveLeft, [&times0]() { times0++; });
    // Nothing should happend as the context are not on the stack
    input.handle_input(f11);
...
TEST_CASE("attempting to clear context from all bindings")
{
    ...
    input.fast_bind_key(EKeyContext::Agent, SDL_SCANCODE_F11, Action::MoveLeft, [&times]() { times++; });
    // Nothing should happend as the context are not on the stack
    input.handle_input(f11);
    CHECK(times == 0);
    input.add_context(EKeyContext::Agent);
...
TEST_CASE("attempting to go back to default context")
{
    ...
    input.fast_bind_key(EKeyContext::DefaultContext, SDL_SCANCODE_F11, Action::MoveLeft, [&times0]() { times0++; });
    input.fast_bind_key(EKeyContext::Agent, SDL_SCANCODE_F11, Action::MoveLeft, [&times1]() { times1++; });
    input.fast_bind_key(EKeyContext::AgentOnHover, SDL_SCANCODE_F11, Action::MoveLeft, [&times2]() { times2++; });
