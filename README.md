...
* Todo Create a binary download link
* Perhaps use the one for User Testing, did anything change since then?

You have two options when installing CultSim, you can download the binary that allows for lua API, and normal use of the program. However if you feel like CultSim is limiting you in any way, or there is some weird behaviors, you might want to build it from source. If you do add more features or fix some bugs, we would be happy if you created push requests with the changes, so that we might include them in future versions of CultSim.

### Binary

CultSim is compiled as 64-bit binary, and thus is only supported as such. If you wish for a 32-bit binary you need to compile it yourself, and you will need to fetch the 32-bit versions of the applicable libaries CultSim depends on.

#### Windows
For most users, unziping the binary `.zip` file, running [vc_redist.x64.exe](https://support.microsoft.com/en-us/help/2977003/the-latest-supported-visual-c-downloads) and making sure you graphics card is up to date should be all that is needed. However we have come across an issue with AMD graphics card needing some additional dependency, however we have yet to isolate what that is, so post an issue if you can't get it up and running.
...
2. Generate the project files using CMake 3.15 or later 
    * If you are using the GUI version, browse to the source and build directories you want in the top of the interface. The source directory should be the folder you cloned, which contains the root `CMakeLists.txt` file. 
...
4. Go to `(Folder you cloned the project into)/out/build/(version you build, debug or release)/_deps/sdl2_external-build`
    * Some CMake configurations do not automtically diferentiate between Release/Debug, so you might find it directly in the build folder.
5. Copy the `SDL2.dll` file
6. Go to `(Folder you cloned the project into)/out/build/(version you build, debug or release)/bachelor/sim`
     * See above for CMake config specific changes to this step
8. Paste the ``.dll` file here
9. repeat steps 3-6 for `physfs.dll` in `(Folder you cloned the project into)/out/build/(version you build, debug or release)/_deps/physfs_external-build`
...
## 3rd party libaries used in CultSim
...
* Used as a replacement to `std::unordered_map` as this implemenation of a hash map is *a lot* faster, and the difference could not be neglected when measuring
...
* Used internally for benchmarking Lua vs Python and other micro benchmarks under way
